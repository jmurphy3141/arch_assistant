# Task p38b: Diagram Architect Hat — Rename + Deep Upgrade

## Goal

Rename `agent/hats/diagram_builder.md` to `agent/hats/diagram_for_oci.md` and
rewrite its content with deep OCI topology rules, draw.io constraints, and
node-placement guidance that a senior OCI architect would apply.

---

## Scope

**Only touch:** `agent/hats/diagram_builder.md` (renamed to `diagram_for_oci.md`).  
**Do NOT touch:** Python files, tests, other hats, or the draw.io generator.
...
grep "diagram_builder" agent/archie_loop.py  # should be zero
```
...
### Step 2 — Rewrite `diagram_for_oci.md`

Replace the full file with the content below.
...
1. `git mv` completed: `diagram_builder.md` gone, `diagram_for_oci.md` present.
2. `python3.11 -c "import agent.hat_engine as h; assert 'diagram_for_oci' in h.load_hats(); print('OK')"` — passes.
3. `grep "parent.*1\|flat.*draw\|IGW\|subnet_tier" agent/hats/diagram_for_oci.md` — multiple matches.
...
```
p38b: rename diagram_builder → diagram_for_oci; OCI topology + draw.io constraint upgrade
```

Branch: `claude/p38b` (from main). Push when done.
