# Task p38e: Create OCI POV Writer Hat (New File)

## Goal

Create `agent/hats/oci_customer_pov_writer.md` — a new hat file that gives the
orchestrator deep POV authoring expertise: discovery mode when context is sparse,
structured document format, OCI competitive positioning, and industry-specific
narrative guidance.
...
**Create new file:** `agent/hats/oci_customer_pov_writer.md`  
**Do NOT touch:** `agent/pov_agent.py`, Python files, other hats, or tests.
...
Create `agent/hats/oci_customer_pov_writer.md` with the full content below.
...
1. File created: `agent/hats/oci_customer_pov_writer.md` exists.
2. `python3.11 -c "import agent.hat_engine as h; assert 'oci_customer_pov_writer' in h.load_hats(); print('OK')"`.
3. `grep "discovery mode\|need_clarification\|Exadata\|measurable" agent/hats/oci_customer_pov_writer.md` — multiple matches.
...
```
p38e: create oci_customer_pov_writer hat — discovery mode, OCI competitive narrative
```

Branch: `claude/p38e` (from main). Push when done.
