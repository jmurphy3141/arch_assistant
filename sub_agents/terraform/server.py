...
def _strip_fences(text: str) -> str:
    ...
    if cleaned.startswith("```"):
        cleaned = re.sub(r"^```(?:json|hcl|terraform|markdown)?\s*", "", cleaned, flags=re.IGNORECASE)
...
def _build_prompt(req: A2ARequest) -> str:
    ...
    if context.get("region"):
        parts.append(f"Target region: {context['region']}")
    if context.get("compartment_id"):
        parts.append(f"Compartment OCID: {context['compartment_id']}")
    parts.append(
        "Return a JSON object with string keys main_tf, variables_tf, "
        "outputs_tf, and readme_md. Do not wrap the JSON in markdown fences."
    )
